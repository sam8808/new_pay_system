...
class TinkoffGatewayController extends Controller
{
    ...
    {
        // Extract request data (example: amount, order ID, etc.)
        $amount = $request['amount']; // Amount in kopecks (e.g., 100 rub = 10000)
        $orderId = $request['order_id']; // Unique order ID
        $description = $request['description']; // Optional description

        // Tinkoff API credentials
        $terminalKey = env('TINKOFF_TERMINAL_KEY'); // Add this to your .env file
        $password = env('TINKOFF_PASSWORD'); // Add this to your .env file
        ...
        $data = [
            ...
            'NotificationURL' => route('tinkoff.webhook'), // Your webhook route
            'SuccessURL' => route('tinkoff.success'), // Your success route
            'FailURL' => route('tinkoff.fail'), // Your failure route
        ];
